...
SELECT COUNT(*) AS total_rows_ragenretings FROM ratings;
...
-- Type your code below:
select count(*) as total_null_id from imdb.movie where id is null;
select count(*) as total_null_year from imdb.movie where year is null;
select count(*) as total_null_datepublished from imdb.movie where date_published is null;
select count(*) as total_null_duration from imdb.movie where duration is null;
select count(*) as total_null_country from imdb.movie where country is null;
select count(*) as total_null_worlwidegrossincome from imdb.movie where worlwide_gross_income is null;
select count(*) as total_null_languages from imdb.movie where languages is null;
select count(*) as total_null_productioncompany from imdb.movie where production_company is null;
...
select count(movie_id) as total_genre_count
from summary;
...
from genre
group by genre)
select * from abc where lower(genre) = 'thriller';
other method
select genre, movie_count,genre_rank
from (select genre,count(movie_id) as movie_count,
rank() OVER  (order by count(movie_id) desc) as genre_rank
from genre
group by genre) as m
...
min(median_rating)as max_median_rating, max(median_rating) as max_median_rating,
...
WITH DirectorMovies AS (
    SELECT
        ...
        r.total_votes,
        r.median_rating,
        LAG(m.date_published) OVER (PARTITION BY dm.name_id ORDER BY m.date_published) AS prev_date_published
    FROM
        director_mapping dm
    JOIN
...
DirectorStats AS (
    SELECT
        ...
        ROUND(AVG(DATEDIFF(date_published, prev_date_published)), 2) AS avg_inter_movie_days,
...
SELECT
    ...
    director_name,
    number_of_movies,
    IFNULL(avg_inter_movie_days, 0) AS avg_inter_movie_days,
    avg_rating,
    total_votes,
    min_rating,
    max_rating,
